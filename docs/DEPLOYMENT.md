...
The build command is `npm ci --include=dev && npm run build`, pre-deploy command is
`npm run db:deploy`, and start command is `npm run start`. Next.js reads the
host-provided `PORT`; no custom server is required.

The repository pins Render and CI to Node `22.22.0`. The explicit npm
`--include=dev` prevents a build-time `NODE_ENV=production` from omitting the
TypeScript, Tailwind, and other compile-time packages. Prisma Client is
